...
Why this exists
---------------
`gossip` can put correspondence in front of a session, but only through the hook surface, and
...
# tools.session_bus.channel) or standing alone next to gossip in an installed plugin directory.
...
NUDGE_TEXT = (
    ...
    "`python -m gossip.bus drain` -- then act on what it returns. "
